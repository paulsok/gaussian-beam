...
figure;
subplot(5,1,1), plot(xxx,Int1(N*0.5,:));
subplot(5,1,2), plot(xxx,Int2(N*0.5,:));
subplot(5,1,3), plot(xxx,Int3(N*0.5,:));
subplot(5,1,4), plot(Q,absF0(N*0.5,:));
subplot(5,1,5), plot(Q,abs(Rs(N*0.5,:)).^2,Q,abs(Rp(N*0.5,:)).^2,Q,absF0(N*0.5,:)/mF);
...
figure
imshow(Int4/maxy);
imwrite(Int4/maxy,'Gauss_steklo-fotorezist.bmp')
mesh(X*a,Y*a,Int4);
...
%for j = 1:13
 %   ezsurf(subs(real(II(j)),[f r],[MyAtan(x,y) sqrt(x^2+y^2)]),[-1 1 -1 1],'circ');
  %  view(0,90)
  %  Filmez(j) = getframe;
%end
%movie(Filmez,20)
